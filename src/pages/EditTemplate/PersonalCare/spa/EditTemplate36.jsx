...
// App Component
const SixthSpa = ({ userDetails, subdomain }) => {
  ...
  const filteredWorks =
    activeFilter === "All Items"
      ...
      : works.filter(
          (work) =>
            sanitizeContent(details?.Blog?.[work.categoryKey]) === activeFilter
        );
          ...
          <div className="lg:w-2/3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
            {treatments.map((treatment, index) => (
              ...
              >
                ...
                <EditTemplateLongInput
                  value={sanitizeContent(
                    details?.Patrners?.[`sectionImage${index + 2}`]
                  )}
                  onChange={(event) =>
                    handleContentChange(
                      "Patrners",
                      `sectionImage${index + 2}`,
                      event
                    )
                  }
                  placeholder="Enter your domain..."
                ...
                <EditTemplateLongInput
                  value={sanitizeContent(
                    details?.Patrners?.[`section${index + 2}header`]
                  )}
                  onChange={(event) =>
                    handleContentChange(
                      "Patrners",
      ...
      >
        <div className="flex flex-col lg:flex-row items-center justify-between gap-10 relative z-10">
          ...
          <div className="relative flex-1">
            <div className=" relative rounded-full overflow-hidden border-8 border-white w-150 h-150 mx-auto lg:mx-0">
              <img
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredWorks.length > 0 ? (
            filteredWorks.map((work, index) => (
              <div
                key={index}
                className="bg-white rounded-[20px] overflow-hidden shadow-sm relative"
              >
                <img
                  src={sanitizeContent(details?.Blog?.[work.imageKey])}
                  alt={sanitizeContent(details?.Blog?.[work.titleKey])}
                ...
                {userPlan && userPermissions?.canEditImage && (
                  <div className="absolute top-4 left-4 z-50">
                    <ButtonSmallPurple
                      onClick={() =>
                        fileInputRefs.current[`Blog-${work.imageKey}`]?.click()
                      }
                    >
                      {loadingImage ? <LoadingSmall /> : "Edit Image"}
                    </ButtonSmallPurple>
                    <input
                      type="file"
                      ref={(ref) =>
                        (fileInputRefs.current[`Blog-${work.imageKey}`] = ref)
                      }
                      onChange={(e) =>
                        handleImageChange(e, "Blog", work.imageKey)
                      }
                      style={{ display: "none" }}
                    />
                  </div>
                ...
                <div className="py-5 px-4">
                  <EditTemplateLongInput
                    value={sanitizeContent(details?.Blog?.[work.categoryKey])}
                    onChange={(e) =>
                      handleContentChange("Blog", work.categoryKey, e)
                    }
                    placeholder="Enter category"
                    className="text-xs text-[#E2836A] uppercase tracking-wide mb-1"
                  />
                  {sanitizeContent(details?.Blog?.[work.titleKey])}
                  <EditTemplateLongInput
                    value={sanitizeContent(details?.Blog?.[work.titleKey])}
                    onChange={(e) =>
                      handleContentChange("Blog", work.titleKey, e)
                    }
                    placeholder="Enter title"
                    className="text-base font-semibold text-[#0c1234]"
                  />
                </div>
              </div>
            ))
      ...
      <div className="relative bg-[#FAF5EF] py-16 px-4 sm:px-6 lg:px-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {authors.map((author, index) => (
            ...
            >
              {/* Rating Section - Consider making rating dynamic if data is available */}
              ...
              <p className="text-gray-600 mb-4 text-sm md:text-base group-hover:text-white">
                {author.testimonial || "No testimonial provided"}
              </p>
              ...
              <div className="absolute bottom-[-60px] left-1/2 transform -translate-x-1/2 text-center">
                <div className="relative">
                  <img
                    src={author.image || "/default-avatar.png"} // Fallback image
                ...
                <p className="text-xs text-gray-500">
                  {author.title || "No title provided"}
                </p>
              </div>

              {/* Edit Inputs - Consider moving these to a separate edit panel */}
              {userPlan && userPermissions?.canEditContent && (
                <>
                  <EditTemplateLongInput
                    value={sanitizeContent(
        ...
        <footer className="bg-blue-950 text-white pt-40 md:pt-48 pb-10 px-6 md:px-20 relative mt-[-120px] md:mt-[-80px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
            <div>
              ...
              />
              <p className="text-sm mb-2"> {userDetails?.phoneNumber}</p>
