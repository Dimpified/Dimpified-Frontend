...
const FindMakeup = ({ subdomain }) => {
  ...
  // Fetch countries and random Makeups on mount
  useEffect(() => {
  ...
  // Fetch random Makeups
  const fetchRandomMakeups = async () => {
    setIsLoading(true);
    try {
      const data = await api.getEcosystemsNearMe({
        ...
        SubCategory: "Makeup Artist Services",
        format: "Onsite",
      });
      const randomMakeups =
        data.ecosystems?.sort(() => 0.5 - Math.random()) || [];
      setEcosystems(randomMakeups);
    } catch (err) {
      setError(err.message || "Failed to fetch Makeup artists.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    try {
      ...
      if (data?.ecosystems?.length) {
        ...
        if (filteredEcosystems.length > 0) {
          ...
          setNotification(
            " No Makeup artist found near you, kindly search nearer local government or state."
          );
          setEcosystems([]);
        }
      } else {
  ...
  return (
    ...
    >
      <Helmet>
        <title>Find a make-up artist near you </title>
        <meta
          name="description"
          content="Find a make-up artist near you with Dimpified "
        />
        <meta property="og:title" content=" Find a make-up artist near you " />
        <meta
      ...
      <div className="font-sen">
        ...
        <section
          ...
          style={{
            backgroundImage: `url("https://gfa-tech.com/dimp-template-images/make-up/beauty/beauty15.jpg")`,
          }}
        >
        ...
        <div className="p-8 px-4 lg:px-32 md:px-24">
          <h1 className="text-2xl text-black font-bold pb-6">
            {notification ? (
              ...
              <>
                Recommended makeup artist services{" "}
