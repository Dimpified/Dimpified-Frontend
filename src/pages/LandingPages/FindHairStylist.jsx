...
const FindHairStylist = ({ subdomain }) => {
  ...
  // Fetch countries and random hair stylist on mount
  useEffect(() => {
    const countriesList = Country.getAllCountries();
    setCountries(countriesList);
    fetchRandomHairStylists();
  }, []);

  // Fetch random hair stylist
  const fetchRandomHairStylists = async () => {
    setIsLoading(true);
    try {
      ...
      const randomHairStylists =
        data.ecosystems?.sort(() => 0.5 - Math.random()) || [];
      setEcosystems(randomHairStylists);
    } catch (err) {
      setError(err.message || "Failed to fetch hair stylist.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    try {
      ...
      if (data?.ecosystems?.length) {
        ...
        if (filteredEcosystems.length > 0) {
          ...
          setNotification(
            " No hair stylist found near you, kindly search nearer local government or state."
          );
          setEcosystems([]);
        }
      } else {
  ...
  return (
    ...
    >
      <Helmet>
              <title>Find a hairdresser near you  </title>
              <meta name="description" content="Find a hairdresser near you with Dimpified " />
              <meta property="og:title" content=" Find a hairdresser near you " />
              <meta
                property="og:description"
                content="Find a hairdresser near you with Dimpified "
              />
            </Helmet>
        ...
        <div className="p-8 px-4 lg:px-32 md:px-24">
          <h1 className="text-2xl text-black font-bold pb-6">
            {notification ? (
              ...
              <>
                Recommended hair stylist services{" "}
...
export default FindHairStylist;
