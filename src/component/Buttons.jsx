...
export const ButtonSmallWhite = ({
  ...
  return (
    <button
      className={`${width} font-body h-${height} rounded p-${padding} text-primary4 bg-primary1 border border-primary3 opacity-80 hover:opacity-100 ${
        disabled ? "opacity-70" : "opacity-100"
      } ${className}`}
      onClick={onClick}
      type={type}
...
export const ButtonForTabs = ({
  ...
  return (
    <button
      type={type}
      className={`text-primary3 ${className} font-body`}
      onClick={onClick}
      disabled={disabled}
    >
      {label}
