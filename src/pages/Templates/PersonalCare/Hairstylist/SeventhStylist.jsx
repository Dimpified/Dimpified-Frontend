import { useState, useEffect, useRef } from "react";
import { Swiper, SwiperSlide } from "swiper/react";
import { Navigation } from "swiper/modules";
import "swiper/css";
import "swiper/css/navigation";
import {
  FaFacebook,
  FaTwitter,
  FaInstagram,
  FaLinkedin,
  FaPhone,
  FaEnvelope,
  FaMapPin,
  FaPaperPlane,
  FaCheckCircle,
  FaThLarge,
  FaShoppingCart,
  FaStar,
  FaRegStar,
  FaBars,
} from "react-icons/fa";
import {
  FaX,
  FaMagnifyingGlass,
  FaArrowLeft,
  FaArrowRight,
  FaPlus,
} from "react-icons/fa6";
import { WhiteContactForm } from "../../../../features/ContactForm/ContactForm";

import { HairSalon } from "../../../../data/Services";
import { useCountry } from "../../../pricing/CountryContext";
import { getFormattedPrice } from "../../../../data/getServicePriceAndCountryCode";
import BookingModal from "../../../../features/Booking/NewBookingModal";
import { GiHairStrands } from "react-icons/gi";

const faqs = [
  {
    question: "What services do you offer?",
    answer:
      "We offer a wide range of beauty and haircare services tailored to your needs.",
  },
  {
    question: "Do I need to book an appointment?",
    answer: "Booking an appointment is recommended to ensure availability.",
  },
  {
    question: "Can you help me decide on a new hairstyle?",
    answer:
      "Absolutely! Our expert stylists will consult with you to find the perfect look that suits your preferences and features.",
  },
  {
    question: "How can I book an appointment?",
    answer: "You can book online through our website or call us directly.",
  },
  {
    question: "What products do you use?",
    answer: "We use only premium, salon-grade products for all our services.",
  },
];

// Data for experts
const expertsData = [
  {
    image: "https://i.imghippo.com/files/QcC3121Yc.jpg", 
    name: "Amaka Okafor",
    position: "Senior Hair Stylist",
  },
  {
    image: "https://i.imghippo.com/files/nHr5073ucE.jpg", 
    name: "Chiamaka Eze",
    position: "Senior Hair Stylist",
  },
  {
    image: "https://i.imghippo.com/files/Aoe4632daw.jpg", 
    name: "Ngozi Balogun",
    position: "Senior Hair Stylist",
  },
];

// Data for testimonials
const testimonialsData = [
  {
    text: "I had the privilege of working with HairGlam on a complex styling session. Their professionalism, attention to detail, and strategic approach resulted in a stunning transformation that exceeded my expectations.",
    name: "Kende Attila",
    position: "Software Tester",
    image: "https://randomuser.me/api/portraits/women/21.jpg",
  },
  {
    text: "The team at HairGlam is exceptional. They took the time to understand my needs and delivered a hairstyle that perfectly matched my personality. I couldn't be happier with the results!",
    name: "Sarah Johnson",
    position: "Marketing Director",
    image: "https://randomuser.me/api/portraits/women/45.jpg",
  },
  {
    text: "From the moment I walked in, I knew I was in good hands. The stylists at HairGlam are true artists who care about their craft. My hair has never looked better!",
    name: "Michael Chen",
    position: "Graphic Designer",
    image: "https://randomuser.me/api/portraits/women/33.jpg",
  },
  {
    text: "HairGlam transformed my damaged hair into a healthy, shiny masterpiece. Their knowledge of hair care and styling techniques is unmatched. Highly recommend!",
    name: "Emily Rodriguez",
    position: "Teacher",
    image: "https://randomuser.me/api/portraits/women/12.jpg",
  },
];



const SeventhStylist = ({userDetails}) => {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [openIndex, setOpenIndex] = useState(2);
  const leftRef = useRef(null);
  const rightRef = useRef(null);

  const toggleFAQ = (index) => {
    setOpenIndex(openIndex === index ? null : index);
  };
  const [isModalOpen, setIsModalOpen] = useState(false); // State to control modal visibility

  const handleModalOpen = () => {
    setIsModalOpen(true);
  };

  const handleModalClose = () => {
    setIsModalOpen(false);
  };
  const { country } = useCountry();
  const countryCode = country || "NG";

  useEffect(() => {
    // Sync heights of left and right refs
    const syncHeight = () => {
      if (window.innerWidth >= 1024) {
        const leftHeight = leftRef.current?.offsetHeight;
        if (rightRef.current) rightRef.current.style.height = `${leftHeight}px`;
      } else {
        if (rightRef.current) rightRef.current.style.height = "auto";
      }
    };

    syncHeight();
    window.addEventListener("resize", syncHeight);
    return () => window.removeEventListener("resize", syncHeight);
  }, []);

  useEffect(() => {
    // Initialize counters
    const counters = [
      { id: "odometer1", target: 25 },
      { id: "odometer2", target: 150 },
      { id: "odometer3", target: 50 },
      { id: "odometer4", target: 10 },
    ];

    counters.forEach((counter) => {
      let current = 0;
      const increment = counter.target / 100;
      const element = document.getElementById(counter.id);

      if (element) {
        const updateCounter = () => {
          current += increment;
          if (current < counter.target) {
            element.textContent = Math.floor(current) + "+";
            setTimeout(updateCounter, 20);
          } else {
            element.textContent =
              counter.target + (counter.id === "odometer4" ? "k+" : "+");
          }
        };

        updateCounter();
      }
    });

    // Set current year in footer
    const yearElement = document.getElementById("year");
    if (yearElement) {
      yearElement.textContent = new Date().getFullYear();
    }
  }, []);

  return (
    <div className="font-sans text-gray-800">
      {/* Mobile Menu */}
      <header className="w-full z-30 bg-[#1c1b1a] text-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          {/* Logo */}
          <a href="#home" className="flex items-center text-xl gap-2">
            <GiHairStrands className="text-[#c59c7b]" />
            HairGlam
          </a>
          {/* Desktop Menu */}
          <ul className="hidden lg:flex items-center gap-6 text-xl">
            <li>
              <a
                href="#home"
                class="hover:text-[#c59c7b] transition-colors duration-200"
              >
                Home
              </a>
            </li>
            <li>
              <a
                href="#about"
                class="hover:text-[#c59c7b] transition-colors duration-200"
              >
                About Us
              </a>
            </li>
            <li>
              <a
                href="#services"
                class="hover:text-[#c59c7b] transition-colors duration-200"
              >
                Services
              </a>
            </li>
            <li>
              <a
                href="#pricing"
                class="hover:text-[#c59c7b] transition-colors duration-200"
              >
                Pricing
              </a>
            </li>
            <li>
              <a
                href="#contact"
                class="hover:text-[#c59c7b] transition-colors duration-200"
              >
                Contact
              </a>
            </li>
          </ul>

          {/* Icons and CTA */}
          <div className="flex items-center gap-4">
            <button
              onClick={handleModalOpen}
              className="bg-[#c59c7b] px-6 py-2 text-sm text-black font-serif hover:opacity-90"
            >
              BOOK NOW
            </button>
          </div>
        </div>
      </header>

      {/* Mobile Menu */}
      <div
        className={`fixed inset-0 z-50 bg-gray-900 p-6 transform transition-transform duration-300 ${
          mobileMenuOpen ? "translate-x-0" : "-translate-x-full"
        } lg:hidden`}
      >
        <div className="flex justify-between items-center mb-6">
          <a href="#home" className="flex items-center text-xl gap-2">
            <GiHairStrands className="text-[#c59c7b]" />
            HairGlam
          </a>
        </div>

        <ul className="space-y-4 text-white text-lg">
          <li>
            <a
              href="#home"
              class="hover:text-[#c59c7b] transition-colors duration-200"
            >
              Home
            </a>
          </li>
          <li>
            <a
              href="#about"
              class="hover:text-[#c59c7b] transition-colors duration-200"
            >
              About Us
            </a>
          </li>
          <li>
            <a
              href="#services"
              class="hover:text-[#c59c7b] transition-colors duration-200"
            >
              Services
            </a>
          </li>
          <li>
            <a
              href="#pricing"
              class="hover:text-[#c59c7b] transition-colors duration-200"
            >
              Pricing
            </a>
          </li>
          <li>
            <a
              href="#contact"
              class="hover:text-[#c59c7b] transition-colors duration-200"
            >
              Contact
            </a>
          </li>
        </ul>

        <button
          onClick={handleModalOpen}
          className="block text-center mt-6 py-2 px-4 bg-[#c59c7b] text-black font-serif"
        >
          BOOK NOW
        </button>
      </div>

      {/* Mobile Overlay */}
      {mobileMenuOpen && (
        <div
          className="fixed inset-0 bg-black bg-opacity-60 z-40 lg:hidden"
          onClick={() => setMobileMenuOpen(false)}
        />
      )}

      {/* Banner Section */}
      <section
        id="home"
        className="relative min-h-screen flex flex-col lg:flex-row items-center pt-20 sm:pt-32 bg-black text-white overflow-hidden"
      >
        {/* Left Panel */}
        <div className="lg:w-1/2 px-6 sm:px-12 z-10">
          <p className="italic text-[#d4a373] mb-2 text-sm">
            Transform Your Style Today
          </p>
          <h1 className="text-4xl sm:text-5xl font-bold leading-tight uppercase">
            Creating Styles,
            <br />
            Inspiring Smiles
          </h1>
          <p className="text-lg text-white mt-6 max-w-md">
            We believe every strand tells a story, and we're here to help you
            tell yours with elegance and style.
          </p>
        </div>

        {/* Right Image */}
        <div className="lg:w-1/2 mt-10 lg:mt-0 relative z-10">
          <img
            src="https://i.imghippo.com/files/ACJA3761w.jpg"
            alt="Hairstylist"
            className="w-full h-[500px] object-cover"
          />
        </div>

        {/* Vertical Text */}
        <div className="absolute left-0 top-1/2 transform -translate-y-1/2 rotate-[-90deg] origin-left hidden lg:block">
          <p className="text-white font-semibold tracking-widest text-xl pl-4">
            YOUR HAIR, YOUR STYLE
          </p>
        </div>

        {/* Scroll Down Arrow */}
        <div className="absolute left-8 bottom-6 lg:block hidden">
          <div className="w-10 h-10 rounded-full border border-white flex items-center justify-center">
            <svg
              className="w-4 h-4 text-white animate-bounce"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              viewBox="0 0 24 24"
            >
              <path d="M19 9l-7 7-7-7" />
            </svg>
          </div>
        </div>
      </section>

      {/* Counter Section */}
      <section className="bg-[#d4a373] py-12">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 text-center gap-6">
            <div>
              <p className="text-3xl font-bold text-[#1e1e1e]">25+</p>
              <p className="text-[#1e1e1e] text-sm mt-2">
                Years of experiences
              </p>
            </div>
            <div>
              <p className="text-3xl font-bold text-[#1e1e1e]">150+</p>
              <p className="text-[#1e1e1e] text-sm mt-2">Unique Services</p>
            </div>
            <div>
              <p className="text-3xl font-bold text-[#1e1e1e]">50+</p>
              <p className="text-[#1e1e1e] text-sm mt-2">Expert Stylists</p>
            </div>
            <div>
              <p className="text-3xl font-bold text-[#1e1e1e]">10k+</p>
              <p className="text-[#1e1e1e] text-sm mt-2">Happy Clients</p>
            </div>
          </div>
        </div>
      </section>

      {/* About Section */}
      <section id="about" className="about py-20 relative">
        <div className="about-inner">
          <div className="flex flex-col h-full  py-4 px-4 lg:px-24">
            <div className="flex flex-col lg:flex-row gap-8">
              <div className="lg:w-1/2">
                <img
                  src="https://i.imghippo.com/files/hFWb9257k.jpg"
                  className="about-img w-[500px] h-[500px] rounded-lg shadow-lg"
                  alt="About HairGlam"
                />
              </div>
              {isModalOpen && (
                <BookingModal
                  isOpen={isModalOpen}
                  handleClose={handleModalClose}
                />
              )}

              <div className="lg:w-1/2 relative z-10">
                <div className="about-content">
                  <span className="sub-heading block text-[#D2B095] mb-2">
                    Our Story, Your Style
                  </span>
                  <h2 className="text-3xl font-bold mb-4 uppercase">
                    Transforming Hair Style Empowering Beauty
                  </h2>
                  <p className="mb-6 max-w-lg">
                    We believe beauty is more than skin deep—it's a reflection
                    of your inner confidence and individuality, one style at a
                    time.
                  </p>

                  <ul className="space-y-3 mb-8">
                    <li className="flex items-center gap-2">
                      <FaCheckCircle className="text-[#D2B095]" />{" "}
                      <span>Personalized Styling</span>
                    </li>
                    <li className="flex items-center gap-2">
                      <FaCheckCircle className="text-[#D2B095]" />{" "}
                      <span>Flexible Booking Options</span>
                    </li>
                    <li className="flex items-center gap-2">
                      <FaCheckCircle className="text-[#D2B095]" />{" "}
                      <span>Satisfaction Guarantee</span>
                    </li>
                  </ul>

                  <a
                    href="#services"
                    className="bg-[#D2B095] inline-flex items-center font-serif py-3 px-3"
                  >
                    See All Services <FaArrowRight className="ml-2" />
                  </a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* Services Section */}
      <section className="services py-20 bg-[#F7F3F0]" id="services">
        <div className="container mx-auto px-4">
          {/* Section Header */}
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-12 gap-6">
            <div className="lg:w-1/2">
              <span className="block text-[#D2B095] mb-2 italic">
                What We Offer
              </span>
              <h2 className="text-3xl font-bold uppercase text-gray-900">
                Explore Our Wide Range of Premium Services
              </h2>
            </div>
            <div className="lg:w-1/2 text-gray-700 text-base">
              <p className="mb-4">
                We're dedicated to transforming your hair into a masterpiece.
                Our expert stylists offer a wide range of services tailored.
              </p>
              <p>
                Whether you're looking for a quick touch-up or a complete
                makeover, HairGlam is here to deliver an experience.
              </p>
            </div>
          </div>

          {/* Services Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 border-t border-l border-gray-300">
            {HairSalon.map((service, index) => (
              <a
                key={index}
                target="_blank"
                rel="noopener noreferrer"
                className="group border-b border-r border-gray-300 p-6 flex flex-col items-start gap-4 transition-all duration-300 hover:border-2 hover:border-gray-500"
              >
                <img
                  src={service.serviceImage}
                  alt={service.name}
                  className="w-12 h-12 object-cover rounded mb-2"
                />
                <h4 className="text-[16px] font-bold uppercase text-gray-800">
                  {service.name}
                </h4>
                <p className="text-sm text-gray-600">{service.description}</p>
                <button
                  onClick={handleModalOpen}
                  className="text-xl text-gray-400 group-hover:text-gray-800 transition"
                >
                  BOOK NOW →
                </button>
              </a>
            ))}
          </div>
        </div>
      </section>

      {/* Why Choose Section */}
      <section className="whychoose py-20 relative bg-white">
        <div className="flex flex-col h-full  py-4 px-4 lg:px-24">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-10">
            {/* Left Text Box */}
            <div className="bg-[#F7F3F0] p-8 lg:p-12 lg:w-9/12 rounded shadow-sm">
              <span className="italic text-[#B99374] block mb-2">
                Why Choose HairGlam?
              </span>
              <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-4 uppercase">
                Building Trust, One Strand at a Time
              </h2>
              <p className="text-gray-700 mb-6 leading-relaxed max-w-2xl">
                We believe your hair deserves nothing but the best. That’s why
                we offer an unparalleled salon experience, combining expert
                stylists, premium products, and a personalized approach.
              </p>

              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-10 text-gray-800 text-base font-medium mb-6 list-disc list-inside">
                <li>Expert Stylists</li>
                <li>Premium Products</li>
                <li>Personalized Services</li>
                <li>Client–Centric Approach</li>
              </ul>

              <a
                href="#services"
                className="inline-flex items-center bg-[#B99374] text-white px-6 py-3 uppercase tracking-wide text-sm hover:bg-[#a67a5e] transition-all"
              >
                Show Services
              </a>
            </div>

            {/* Right Image */}
            <div className="lg:w-3/12 w-full flex justify-center">
              <img
                src="https://i.imghippo.com/files/QcC3121Yc.jpg"
                alt="Stylist with client"
                className="rounded-md w-full object-cover h-[500px]"
              />
            </div>
          </div>
        </div>
      </section>

      {/* Pricing Section */}
      <section
        id="pricing"
        className="pricing-section relative bg-[#F7F3F0] py-20"
      >
        {/* Vertical “Pricing List” label on XL+ */}
        <div className="right-text hidden xl:block absolute right-8 top-1/2 transform -translate-y-1/2">
          <h2 className="vertical-text uppercase tracking-wider text-gray-300">
            Pricing List
          </h2>
        </div>

        <div className="container mx-auto px-4">
          {/* Header */}
          <div className="text-center max-w-3xl mx-auto mb-12">
            <span className="sub-heading block text-[#D2B095] mb-2 italic">
              Our Service Pricing
            </span>
            <h2 className="text-4xl font-bold uppercase mb-4 text-gray-900">
              Pricing That Works for You
            </h2>
            <p className="text-gray-700">
              We believe in delivering premium salon experiences at prices that
              suit your budget. Our pricing structure is transparent,
              competitive.
            </p>
          </div>

          {/* Two‑column list with centre divider */}
          <div className="flex flex-col lg:flex-row items-start lg:items-stretch gap-8">
            {/* Left column */}
            <div className="lg:w-1/2 space-y-6">
              {HairSalon.slice(0, 6).map((service, index) => (
                <div key={index} className="pb-4">
                  <div className="flex items-center">
                    <p className="name font-medium text-gray-800">
                      {service.name}
                    </p>
                    <span className="flex-grow border-t border-dashed border-gray-400 mx-4"></span>
                    <p className="price font-semibold text-gray-800">
                      {getFormattedPrice(service.price, countryCode)}
                    </p>
                  </div>
                  <p className="text-gray-600 mt-1">
                    {service.shortDescription}
                  </p>
                </div>
              ))}
            </div>

            {/* Vertical divider on lg+ */}
            <div className="hidden lg:block w-px bg-gray-400"></div>

            {/* Right column */}
            <div className="lg:w-1/2 space-y-6">
              {HairSalon.slice(6, 12).map((service, index) => (
                <div key={index} className="pb-4">
                  <div className="flex items-center">
                    <p className="name font-medium text-gray-800">
                      {service.name}
                    </p>
                    <span className="flex-grow border-t border-dashed border-gray-400 mx-4"></span>
                    <p className="price font-semibold text-gray-800">
                      {getFormattedPrice(service.price, countryCode)}
                    </p>
                  </div>
                  <p className="text-gray-600 mt-1">
                    {service.shortDescription}
                  </p>
                </div>
              ))}
            </div>
          </div>
          <div className="flex justify-center mt-12">
            <button
              onClick={handleModalOpen}
              className="inline-flex items-center border border-gray-800 px-6 py-2 uppercase text-sm font-medium tracking-wide hover:bg-gray-800 hover:text-white transition"
            >
              Reserve your spot now
            </button>
          </div>
        </div>
      </section>

      {/* Experts Section */}
      <section
        className="services experts py-20 bg-gray-900 text-white relative"
        id="experts"
      >
        <div className="container mx-auto px-4">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-12 gap-6">
            <div className="lg:w-1/2">
              <span className="sub-heading block text-primary mb-2">
                Our Expert Team
              </span>
              <h2 className="text-2xl font-bold uppercase">
                Talented, Experienced, Trusted
              </h2>
            </div>
            <div className="lg:w-1/2 lg:pl-12">
              <p className="text-white text-opacity-80">
                We take pride in our team of talented, passionate, and <br />
                experienced professionals.
              </p>
            </div>
          </div>

          <Swiper
            modules={[Navigation]}
            spaceBetween={30}
            slidesPerView={1}
            breakpoints={{
              640: { slidesPerView: 2 },
              1024: { slidesPerView: 3 },
            }}
            navigation={{
              prevEl: ".expert-prev",
              nextEl: ".expert-next",
            }}
            className="expertSwiper mb-8"
          >
            {expertsData.map((expert, index) => (
              <SwiperSlide key={index}>
                <div className="expert-card bg-gray-800 rounded-lg overflow-hidden">
                  <div className="img-box relative group">
                    <img
                      src={expert.image}
                      className="w-full h-auto object-cover"
                      alt={expert.name}
                      width="352"
                      height="306"
                    />
                    <div className="social absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition">
                      <ul className="links flex space-x-4 mb-0">
                        <li>
                          <a
                            aria-label="Facebook"
                            href="#"
                            className="text-white hover:text-primary"
                          >
                            <FaFacebook />
                          </a>
                        </li>
                        <li>
                          <a
                            aria-label="Twitter"
                            href="#"
                            className="text-white hover:text-primary"
                          >
                            <FaTwitter />
                          </a>
                        </li>
                      </ul>
                      <button
                        aria-label="show social links"
                        className="social-btn absolute bottom-4 right-4 bg-primary text-white w-10 h-10 rounded-full flex items-center justify-center z-10"
                      >
                        <FaPlus />
                      </button>
                    </div>
                  </div>
                  <div className="card-footer p-4 flex justify-between items-end">
                    <div>
                      <h5 className="font-semibold text-white">
                        {expert.name}
                      </h5>
                      <p className="mb-0 text-white text-opacity-70">
                        {expert.position}
                      </p>
                    </div>
                    <div className="number text-white text-opacity-50 text-xl">
                      {String(index + 1).padStart(2, "0")}
                    </div>
                  </div>
                </div>
              </SwiperSlide>
            ))}
          </Swiper>

          <div className="flex justify-center">
            <div className="flex space-x-4">
              <button className="expert-prev bg-primary text-white w-12 h-12 rounded-full flex items-center justify-center hover:bg-primary-dark transition">
                <FaArrowLeft />
              </button>
              <button className="expert-next bg-primary text-white w-12 h-12 rounded-full flex items-center justify-center hover:bg-primary-dark transition">
                <FaArrowRight />
              </button>
            </div>
          </div>
        </div>
      </section>

      {/* Gallery Section */}
      <section className="gallery py-20 relative">
        <div className="right-text absolute right-8 top-1/2 transform -translate-y-1/2 hidden xl:block">
          <h2 className="vertical-text font-sans uppercase tracking-wider">
            Gallery
          </h2>
        </div>

        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-12">
            <span className="sub-heading block text-primary mb-2">
              Showcase Creativity
            </span>
            <h2 className="text-4xl font-bold uppercase mb-4">
              Highlights of Our Expertise
            </h2>
            <p>
              We believe every hairstyle is a masterpiece, crafted with passion
              and precision. Our <br /> gallery showcases the stunning
              transformations.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div className="sm:col-span-1">
              <a
                href="#"
                className="project-card glightbox block relative group overflow-hidden rounded-lg"
                aria-label="See Original Image"
              >
                <img
                  src="https://i.imghippo.com/files/Aoe4632daw.jpg"
                  width="416"
                  height="768"
                  alt="Salon Gallery"
                  className="w-full h-full object-cover transition duration-500 group-hover:scale-105"
                />
                <div className="info absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition">
                  <div className="plus-icon bg-primary text-white w-12 h-12 rounded-full flex items-center justify-center">
                    <FaPlus />
                  </div>
                </div>
              </a>
            </div>

            <div className="sm:col-span-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="sm:col-span-2">
                  <a
                    href="#"
                    className="project-card glightbox block relative group overflow-hidden rounded-lg"
                    aria-label="See Original Image"
                  >
                    <img
                      src="https://i.imghippo.com/files/Pfrk9132AQs.jpg"
                      width="856"
                      height="372"
                      alt="Salon Gallery"
                      className="w-full h-64 sm:h-80 object-cover transition duration-500 group-hover:scale-105"
                    />
                    <div className="info absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition">
                      <div className="plus-icon bg-primary text-white w-12 h-12 rounded-full flex items-center justify-center">
                        <FaPlus />
                      </div>
                    </div>
                  </a>
                </div>

                <div className="sm:col-span-1">
                  <a
                    href="/assets/images/project-showcase-3.webp"
                    className="project-card glightbox block relative group overflow-hidden rounded-lg"
                    aria-label="See Original Image"
                  >
                    <img
                      src="https://i.imghippo.com/files/lQbn4592w.jpg"
                      width="416"
                      height="372"
                      alt="Salon Gallery"
                      className="w-full h-64 object-cover transition duration-500 group-hover:scale-105"
                    />
                    <div className="info absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition">
                      <div className="plus-icon bg-primary text-white w-12 h-12 rounded-full flex items-center justify-center">
                        <FaPlus />
                      </div>
                    </div>
                  </a>
                </div>

                <div className="sm:col-span-1">
                  <a
                    href="/assets/images/project-showcase-4.webp"
                    className="project-card glightbox block relative group overflow-hidden rounded-lg"
                    aria-label="See Original Image"
                  >
                    <img
                      src="https://i.imghippo.com/files/Xh9929jM.jpg"
                      width="416"
                      height="372"
                      alt="Salon Gallery"
                      className="w-full h-64 object-cover transition duration-500 group-hover:scale-105"
                    />
                    <div className="info absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition">
                      <div className="plus-icon bg-primary text-white w-12 h-12 rounded-full flex items-center justify-center">
                        <FaPlus />
                      </div>
                    </div>
                  </a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* Testimonials Section */}
      <section
        className="testimonial py-20 bg-cover bg-center relative"
        style={{
          backgroundImage:
            "url(https://i.imghippo.com/files/YlE4960aOM.jpg)",
        }}
      >
        <div className="absolute inset-0 bg-black bg-opacity-70"></div>
        <div className="right-text absolute right-8 top-1/2 transform -translate-y-1/2">
          <h2 className="vertical-text font-sans uppercase tracking-wider text-white">
            testimonial
          </h2>
        </div>

        <div className="container mx-auto px-4 relative z-10">
          <div className="flex justify-end">
            <div className="w-full lg:w-10/12 xl:w-9/12 2xl:w-8/12">
              <div className="testimonial-inner bg-white bg-opacity-10 backdrop-blur-sm p-8 lg:p-12 rounded-lg">
                <span className="sub-heading block text-primary mb-2">
                  What Our Clients Say
                </span>
                <h2 className="text-4xl font-bold uppercase text-white mb-4">
                  Clients' Success Stories
                </h2>
                <p className="mb-8 text-white text-opacity-80 max-w-2xl">
                  We pride ourselves on delivering exceptional hair care and
                  unforgettable experiences. But don't just take our word for
                  it—hear from the people who matter most.
                </p>

                <Swiper
                  modules={[Navigation]}
                  spaceBetween={30}
                  slidesPerView={1}
                  navigation={{
                    prevEl: ".client-prev",
                    nextEl: ".client-next",
                  }}
                  className="clientSwiper mb-8"
                >
                  {testimonialsData.map((testimonial, index) => (
                    <SwiperSlide key={index}>
                      <div className="testimonial-card bg-white p-8 rounded-lg relative">
                        <div className="flex gap-2 text-yellow-400">
                          {[...Array(4)].map((_, i) => (
                            <FaStar key={i} />
                          ))}
                          <FaRegStar />
                        </div>
                        <p className="mt-4 mb-6 text-gray-700">
                          {testimonial.text}
                        </p>
                        <div className="flex gap-4 items-center">
                          <img
                            width="60"
                            height="60"
                            src={testimonial.image}
                            alt={testimonial.name}
                            className="rounded-full"
                          />
                          <div>
                            <h5 className="font-semibold mb-1">
                              {testimonial.name}
                            </h5>
                            <span className="text-gray-600">
                              {testimonial.position}
                            </span>
                          </div>
                        </div>
                        <img
                          src="/assets/images/quote.png"
                          alt="Quote"
                          className="absolute top-4 right-4 w-12 opacity-10"
                        />
                      </div>
                    </SwiperSlide>
                  ))}
                </Swiper>

                <div className="flex justify-center space-x-4">
                  <button className="client-prev bg-primary text-white w-12 h-12 rounded-full flex items-center justify-center hover:bg-primary-dark transition">
                    <FaArrowLeft />
                  </button>
                  <button className="client-next bg-primary text-white w-12 h-12 rounded-full flex items-center justify-center hover:bg-primary-dark transition">
                    <FaArrowRight />
                  </button>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* FAQ Section */}
      <section className="w-full bg-[#f9f1eb] text-[#1e1e1e] py-16">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-2 gap-10 items-stretch">
          {/* Left: FAQ */}
          <div ref={leftRef}>
            <p className="italic text-sm text-[#b1907f] mb-2">
              Quick Answers For You
            </p>
            <h2 className="text-3xl md:text-4xl font-bold mb-4">
              FIND THE ANSWERS YOU NEED
            </h2>
            <p className="text-[#5e5e5e] mb-8 max-w-lg">
              we understand that seeking legal representation can raise various
              questions. Below are some of the most frequently asked questions
              to help
            </p>

            <div className="space-y-4">
              {faqs.map((faq, index) => (
                <div key={index}>
                  <button
                    onClick={() => toggleFAQ(index)}
                    className={`w-full text-left px-6 py-4 flex justify-between items-center font-medium text-lg bg-white hover:bg-gray-100 transition ${
                      openIndex === index ? "bg-[#c7a189] text-black" : ""
                    }`}
                  >
                    {faq.question}
                    <span className="text-2xl">
                      {openIndex === index ? "−" : "+"}
                    </span>
                  </button>
                  {openIndex === index && (
                    <div className="bg-[#c7a189] text-black px-6 py-4">
                      <p>{faq.answer}</p>
                    </div>
                  )}
                </div>
              ))}
            </div>
          </div>

          {/* Right: Booking Form + Image */}
          <div
            ref={rightRef}
            className="flex flex-col bg-white shadow-md overflow-hidden"
          >
            {/* Form */}
            <div id="contact">
              <WhiteContactForm />
            </div>

            {/* Image */}
            <div className="w-full h-[220px] lg:h-[280px]">
              <img
                src="https://i.imghippo.com/files/oNU5238RU.jpg"
                alt="Hair Stylist"
                className="w-full h-full object-cover"
              />
            </div>
          </div>
        </div>
      </section>

      {/* Footer */}
      <footer className="bg-[#faf7f5] text-[#1c2536] text-sm leading-relaxed">
        <div className="max-w-7xl mx-auto px-4 lg:px-8 py-16">
          {/* ---------- TOP BLOCK ---------- */}
          <div className="flex flex-col lg:flex-row gap-10">
            {/* LEFT — logo + intro + socials */}
            <div className="lg:w-1/3">
              <a href="/" aria-label="Go to home">
                <a
                  href="#home"
                  className="flex items-center text-2xl font-bold gap-2"
                >
                  <GiHairStrands className="text-[#c59c7b]" />
                  HairGlam
                </a>
              </a>

              <p className="mb-8 max-w-sm">
                Welcome to HairGlam, where legal expertise meets personalised
                service. We are a team of highly skilled attorneys dedicated to
                providing
              </p>

              <ul className="flex gap-4">
                {[
                  { icon: <FaFacebook />, label: "Facebook" },
                  { icon: <FaInstagram />, label: "Instagram" },
                  { icon: <FaLinkedin />, label: "LinkedIn" },
                  { icon: <FaTwitter />, label: "Twitter" },
                ].map(({ icon, label }) => (
                  <li
                    key={label}
                    className="border border-[#d7d3cd] w-10 h-10 grid place-items-center transition hover:bg-[#cbb19c] hover:text-white"
                  >
                    <a href="#" aria-label={label}>
                      {icon}
                    </a>
                  </li>
                ))}
              </ul>
            </div>

            {/* RIGHT — opening hours + link columns */}
            <div className="lg:w-2/3">
              {/* OPENING HOURS */}
              <div className="flex flex-col lg:flex-row items-center gap-6 pb-10 border-b border-[#e6e1da]">
                {/* Icon + heading */}
                <div className="lg:w-1/3 flex items-center gap-4">
                  <img
                    src="https://i.imghippo.com/files/MAv2546HHc.png"
                    alt="Clock icon"
                    className="h-14"
                  />
                  <h2 className="text-2xl font-semibold tracking-wide uppercase">
                    Opening&nbsp;Hour
                  </h2>
                </div>

                {/* Hours grid */}
                <div className="lg:w-2/3 grid sm:grid-cols-2 w-full text-[#585858]">
                  <div className="border-b sm:border-b-0 sm:border-r border-[#e6e1da] py-6 sm:py-0 sm:px-6">
                    <p className="uppercase text-xs mb-1 tracking-wide">
                      Saturday&nbsp;to&nbsp;Thursday
                    </p>
                    <p className="text-lg font-medium text-[#1c2536]">
                      09:30&nbsp;AM&nbsp;‑&nbsp;20:30&nbsp;PM
                    </p>
                  </div>
                  <div className="py-6 sm:py-0 sm:px-6">
                    <p className="uppercase text-xs mb-1 tracking-wide">
                      Friday
                    </p>
                    <p className="text-lg font-medium text-[#1c2536]">
                      02:30&nbsp;AM&nbsp;‑&nbsp;19:00&nbsp;PM
                    </p>
                  </div>
                </div>
              </div>

              {/* QUICK‑LINK / SHOP / CONTACT / NEWSLETTER */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-10 pt-10">
                {/* quick link */}
                <div>
                  <h4 className="mb-5 font-semibold uppercase tracking-wide">
                    Quick&nbsp;Link
                  </h4>
                  <ul className="space-y-2">
                    {[
                      "Home",
                      "About Us",
                      "Service",
                      "Pricing",
                      "Contact us",
                    ].map((item) => (
                      <li key={item}>
                        <a
                          href="#"
                          className="hover:text-[#c09f84] transition-colors"
                        >
                          {item}
                        </a>
                      </li>
                    ))}
                  </ul>
                </div>

                {/* contact */}
                <div>
                  <h4 className="mb-5 font-semibold uppercase tracking-wide">
                    Contact
                  </h4>
                  <ul className="space-y-5">
                    <li className="flex gap-3">
                      <FaPhone className="min-w-5 mt-1 text-[#c09f84]" />
                      <div>
                        <a
                          href="tel:8085550111"
                          className="block hover:text-[#c09f84]"
                        >
                          (808)&nbsp;555‑0111
                        </a>
                        <a
                          href="tel:3025550107"
                          className="block hover:text-[#c09f84]"
                        >
                          (302)&nbsp;555‑0107
                        </a>
                      </div>
                    </li>
                    <li className="flex gap-3">
                      <FaEnvelope className="min-w-5 mt-1 text-[#c09f84]" />
                      <div>
                        <a
                          href="mailto:info@example.com"
                          className="block hover:text-[#c09f84]"
                        >
                          info@example.com
                        </a>
                        <a
                          href="mailto:info@example.com"
                          className="block hover:text-[#c09f84]"
                        >
                          info@example.com
                        </a>
                      </div>
                    </li>
                    <li className="flex gap-3">
                      <FaMapPin className="min-w-5 mt-1 text-[#c09f84]" />
                      <div>
                        <p>3605 Parker&nbsp;Rd.</p>
                        <p>3890 Poplar&nbsp;Dr.</p>
                      </div>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>

          {/* ---------- BOTTOM STRIP ---------- */}
          <div className="mt-14 pt-6 border-t border-[#e6e1da] flex flex-wrap items-center justify-between gap-4">
            <p>
              Copyright © <span className="font-semibold">HairGlam</span>{" "}
              {new Date().getFullYear()}. All rights reserved.
            </p>
          </div>
        </div>

        {/* Scroll‑to‑top button (optional) */}
        <a
          href="#"
          aria-label="Scroll to top"
          className="fixed bottom-6 right-6 w-11 h-11 bg-[#c09f84] text-[#1c2536] grid place-items-center shadow-sm hover:opacity-90"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="18"
            height="18"
            fill="currentColor"
            viewBox="0 0 16 16"
          >
            <path
              fillRule="evenodd"
              d="M8.354 3.646a.5.5 0 0 0-.708 0l-5 5a.5.5 0 1 0 .708.708L8 4.707l4.646 4.647a.5.5 0 0 0 .708-.708l-5-5z"
            />
          </svg>
        </a>
      </footer>
    </div>
  );
};

export default SeventhStylist;

