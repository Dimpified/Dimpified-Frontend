...
const FindBarber = ({ subdomain }) => {
  ...
  // Fetch countries and random barbers on mount
  useEffect(() => {
  ...
  // Fetch random barbers
  const fetchRandomBarbers = async () => {
    setIsLoading(true);
    try {
    ...
    try {
      ...
      if (data?.ecosystems?.length) {
        ...
        if (filteredEcosystems.length > 0) {
          ...
          setNotification(
            " No barber found near you, kindly search nearer local government or state."
          );
          setEcosystems([]);
        }
      } else {
